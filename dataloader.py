...
class BiONetDataset(Dataset):
    ...
    def __init__(self, path, data_name, batchsize=1, steps=None, shuffle=False, transforms=None):
        ...
        if steps is not None:
            self.idx_mapping = np.random.randint(0, self.x.shape[0], steps*batchsize)#随机返回数
    ...
    def load_data(self, path,data_name):
        ...
        if data_name == 'monuseg':
            folders = os.listdir(path)#绝对路径返回指定路径下的文件和文件夹列表。
            
            imgs_list = []
            masks_list = []
            for folder in folders:
                items = os.listdir(os.path.join(path,'x'))#os.listdir(）：返回输入路径下的文件和列表名称，os.path.join():拼接待操作对象

                
                for item in items:
                    item1=item[:-len(".bmp")]
                    item2=str(item1)+'_GT.bmp'
                    
                    imgs_list.append(np.array(Image.open(os.path.join(path,'x',item)).convert('RGB')))
                    
                    masks_list.append(np.array(Image.open(os.path.join(path,'y',item2)).convert('L')))
